...
/**
 * Ublox::ubxCfgPrtUsb
 * Set the USB port configuration.
 *
 * @param cfg
 * The configuration. Notice that txready always is disabled.
 *
 * @return
 * true: ack or nak received
 * false: timeout
 ...
 * @return
 */
bool Ublox::ubxCfgPrtUsb(ubx_cfg_prt_usb *cfg)
{
    uint8_t buffer[20];
    int ind = 0;

    ubx_put_U1(buffer, &ind, 3); // ID for USB
    ubx_put_U1(buffer, &ind, 0);
    ubx_put_X2(buffer, &ind, 0); // Always disable txready function
